...
TX_MUTEX sample_mutex;


/* New Thread entry function */
void new_thread4_entry(ULONG entry_input)
{
	UINT status;
	static UINT i;
	CHAR sample_output[30];

	status = tx_mutex_create(&sample_mutex,"sample mutex",TX_INHERIT);

    while (1)
    {
    	status = tx_mutex_get(&sample_mutex,TX_WAIT_FOREVER);
    	//sample_output = "new_thread4_entry hello";
    	strcpy(sample_output,"new_thread4_entry hello");
